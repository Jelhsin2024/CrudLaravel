...
const storeContacto = (req, res) => {
    const {celular, celular2, whatsapp, direccion, descripcion} = req.body;
    const sql = "INSERT INTO contactos (celular, celular2, whatsapp, direccion, descripcion) VALUES (?,?,?,?,?)";
    db.query(sql,[celular, celular2, whatsapp, direccion, descripcion], (error, result) => {
        console.log(result);
        if(error){
...
const updateContacto = (req, res) => {
    ...
    const sql = "UPDATE contactos SET celular = ?, celular2 = ?, whatsapp = ?, direccion = ?, descripcion = ? WHERE id = ?";
    db.query(sql, [celular, celular2, whatsapp, direccion, descripcion, id_contacto], (error, result) => {
        if (error) {
            return res.status(500).json({error: "ERROR: Intente más tarde por favor."});
        }
